...
/// from `orders` / `order_items` / `staff` via the indexes declared in
/// `app_database.dart`, so nothing is duplicated.
library;
...
class SalesEntry {
  ...
  static SalesEntry fromRow(Map<String, Object?> row) => SalesEntry(
...
class SalesQueryRepository {
  ...
  Future<List<SalesEntry>> getSales({
    ...
    final rows = await _db.rawQuery(
      ...
      'o.created_at, o.order_number, oi.product_id, '
