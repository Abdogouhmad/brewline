...
///
/// PINs are stored as salted SHA-256 hashes (see `password_hash.dart`). Each
/// user gets a random per-user salt, so two users who choose the same PIN
/// produce **different** stored hashes. A `UNIQUE` index on the stored hash
/// column would silently pass both rows through — completely defeating the
/// uniqueness requirement without any error.
///
...
///
/// Checks the admin account (via [credentials]) first, then scans the staff
...
Future<PinLookupResult?> findUserByPin(
  ...
  // 1. Admin — the single stored account, in the credential store.
  final admin = await credentials.read();
  if (admin != null && admin.pinHash == hashPin(pin, admin.pinSalt)) {
  ...
  for (final member in activeStaff) {
    if (member.pinHash == hashPin(pin, member.pinSalt)) {
      return PinLookupResult(
        role: Role.waiter,
        userId: member.id,
...
Future<bool> isPinTaken(
  ...
  if (excludingUserId != 'admin') {
    final admin = await credentials.read();
    if (admin != null &&
        admin.pinHash == hashPin(candidatePin, admin.pinSalt)) {
      return true;
    }
  }
  ...
  for (final member in activeStaff) {
    ...
    if (member.pinHash == hashPin(candidatePin, member.pinSalt)) return true;
  }
